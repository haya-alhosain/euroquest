...
export const getBlogs = async (): Promise<BlogSection[]> => {
  try {
    // Check cache first
    const cached = cacheService.get<BlogSection[]>(API_ENDPOINTS.BLOGS);
    if (cached) {
      return cached;
    }

    const response = await apiClient.get<BlogsApiResponse>(API_ENDPOINTS.BLOGS);

    if (response.success && response.data) {
      // Transform the API response to BlogSection format
      const blogSections: BlogSection[] = [];

      // The API returns blogs grouped by tag_name, but the key is empty string for untagged blogs
      Object.entries(response.data.blogs).forEach(([tagName, blogData]) => {
        if (blogData.data && blogData.data.length > 0) {
          blogSections.push({
            tagName: tagName || "Latest Articles", // Use 'Latest Articles' for empty tag names
            blogs: blogData.data,
          });
        }
      });
      ...
      // Cache the result
      cacheService.set(API_ENDPOINTS.BLOGS, blogSections);
      return blogSections;
    }
